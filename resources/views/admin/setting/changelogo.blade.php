        ...
        <div class="box-header">
                ...
                <div class="box">
                    {{ Form::open(array('method' => 'PATCH','files' => true,'url' =>'adminpanel/setapp/'.$changelogo->id )) }}
                        ...
                        <div class="form-group col-md-6">
                        <label>الكوبون </label>
                            <input style="width:100%;" type="text" class="form-control" value='{{$changelogo->copoun}}' name="copoun">
                            @if ($errors->has('copoun'))
                            <div style="color: crimson;font-size: 18px;" class="error">{{ $errors->first('copoun') }}</div>
                            @endif
                        </div>
                        
                        <div class="form-group col-md-6">
                        <label>الخصم </label>
                            <input style="width:100%;" type="number" class="form-control" value='{{$changelogo->discount}}' name="discount">
                            @if ($errors->has('discount'))
                            <div style="color: crimson;font-size: 18px;" class="error">{{ $errors->first('discount') }}</div>
                            @endif
                        </div>
                        <div class="form-group col-md-6">
                        <label>عدد من يحصل على الخصم </label>
                            <input style="width:100%;" type="number" class="form-control" value='{{$changelogo->count}}' name="ncount">
                            @if ($errors->has('ncount'))
                            <div style="color: crimson;font-size: 18px;" class="error">{{ $errors->first('ncount') }}</div>
                            @endif
                        </div>
                        <div class="form-group col-md-6">
                        <label>عدد الحاصلين على الخصم </label>
                            <input style="width:100%;" type="number" class="form-control" value='{{$count}}' name="count" readonly>
